...
/** Load ospf log file in iframe on demand */
$(document).on('click', '#show_ospf', function () {
    if (config_content.is(':hidden')) {
        $.ajax({
            url    : $(this).data('url'),
            type   : 'post',
...
/** Load git log for ospf of current node */
$(document).on('click', '#show_ospf_history, .reload_history', function () {
    if (diff_content.is(':hidden') || this.className === 'reload_history') {
        $.ajax({
            url    : $(this).data('url'),
            type   : 'post',
