...
class Node extends ActiveRecord
{
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getOutOspfBackups()
    {
        return $this->hasMany(OutOspfBackup::class, ['node_id' => 'id']);
    }
     ...
     * @return array|string
     */
    public static function getBackupCommitLog($id, $directory)
    {
        try {

            $wrapper     = new GitWrapper(\Y::param('gitPath'));
            $backup_path = \Y::param('dataPath') . DIRECTORY_SEPARATOR . $directory;
     ...
     * @return array|string
     */
    public static function getCommitMetaData($hash, $directory)
    {
        try {
     ...
     * @return string
     */
    public static function getBackupGitVersion($id, $hash, $directory)
    {
        try {
    ...
    public function getAuthItems() {
      return $this->hasMany(\app\modules\rbac\models\AuthItem::className(), ['name' => 'auth_item_name'])->viaTable('auth_item_node', ['node_id' => 'id']);
    }

    public function getDeniedNodes() {
    ...
    public function isUserAllowed($node_id, $roles) {
      ...
      $getPermission = (new Query())
          ...
          ->from('{{auth_item_node}}')
          ->where('node_id=:node_id', [':node_id' => $node_id])
          ->andWhere(['auth_item_name' => $roles_names])
          ->exists();
