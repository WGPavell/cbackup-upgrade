                ...
                <div class="tab-pane table-responsive" id="ospf_backup_tab">
                    <?php if (!empty($data->outOspfBackups)): ?>
                        <table class="table table-bordered" style="margin-bottom: 0;">
                            ...
                            <tr>
                                <td><?= $data->outOspfBackups[0]->id ?></td>
                                <td>
                                    <?php /** @noinspection PhpUnhandledExceptionInspection */
                                        echo Yii::$app->formatter->asDatetime($data->outOspfBackups[0]->time, 'php:'.Setting::get('datetime'))
                                    ?>
                                </td>
                                <td class="hidden-sm hidden-xs"><?= $data->outOspfBackups[0]->hash ?></td>
                                <td class="hidden-sm hidden-xs">
                                    <?php
                                        $com_status  = 'disabled';
                                        $com_hash    = '';
                                        ...
                                        $disabled    = '';
                                        $path        = Yii::t('network', 'Database');
                                        $file_path   = DIRECTORY_SEPARATOR . 'ospf_backup' . DIRECTORY_SEPARATOR . "{$data->id}.txt";
                                        ...
                                        /** Check if config exists in database or file */
                                        if (($task_info->put == 'file' && !file_exists(Y::param('dataPath') . $file_path)) ||
                                            ($task_info->put == 'db'   && is_null($data->outOspfBackups[0]->ospf_config))) {

                                            $conf_exists = Html::tag('i', '', [
                                                'class'               => 'fa fa-warning text-danger cursor-question',
                                                'title'               => Yii::t('node', 'Ospf backup not found'),
                                                'data-toggle'         => 'tooltip',
                                                'data-placement'      => 'right',
                                            ]);
                                        ...
                                        /** Check if file commits exists in GIT repo */
                                        if ($task_info->put == 'file' && is_array($ospf_commit_log) && array_key_exists('0', $ospf_commit_log) ) {
                                            $com_hash   = $ospf_commit_log[0][0];
                                            $com_status = '';
                                        }
                                ...
                                <td class="narrow">
                                    <?php
                                        echo Html::a('<i class="fa fa-eye"></i> ' .Yii::t('app', 'View'), '#ospf_content', [
                                            'id'          => 'show_ospf',
                                            'class'       => 'btn btn-xs btn-default margin-r-5 ' . $disabled,
                                            'data-toggle' => "collapse",
                                            'data-parent' => '#accordion_ospf',
                                            'data-url'    => Url::to(['ajax-load-ospf']),
                                            'data-params' => json_encode([
                                                'node_id' => $data->id,
                                                'put'     => $task_info->put,
                                            ])
                                        ...
                                        if ($task_info->put == 'file') {
                                            echo Html::a('<i class="fa fa-book"></i> ' . Yii::t('app', 'History'), '#diff_ospf_content', [
                                                'id'          => 'show_ospf_history',
                                                'class'       => 'btn btn-xs btn-default margin-r-5 ' . "{$disabled} {$com_status}",
                                                'data-toggle' => "collapse",
                                                'data-parent' => '#accordion_ospf',
                                                'data-url'    => Url::to(['ajax-load-ospf-diff']),
                                                'data-params' => json_encode([
                                                    'node_id' => $data->id,
                                                    'hash'    => $com_hash,
                                                ])
                                        ...
                                        echo Html::a('<i class="fa fa-download"></i> ' . Yii::t('app', 'Download'), Url::to(['ajax-ospf-download',
                                            'id'   => $data->id,
                                            'put'  => $task_info->put,
                                            'hash' => null,
                                            'directory' => 'ospf_backup'
                                        ]), [
                                            'class'         => 'btn btn-xs btn-default ' . $disabled,
                                            'title'         => Yii::t('app', 'Download'),
                                            'data-dismiss'  => 'modal',
                    ...
                    <?php else: ?>
                        <div class="callout callout-info" style="margin: 10px;">
                            <p><?= Yii::t('node', 'Ospf backup not found') ?></p>
                        </div>
                    <?php endif; ?>

                    <div class="panel-group" id="accordion_ospf" style="margin-bottom: 0">
                        <div class="panel panel-no-border panel-default">
                            <div id="ospf_content" class="panel-collapse collapse">
                                <div class="panel-body">
                                    <?php
                                        echo Html::tag('iframe', '' , [
                                            'id'    => 'ospf_iframe',
                        ...
                        <div class="panel panel-no-border panel-default" style="margin-top: 0">
                            <div id="diff_ospf_content" class="panel-collapse collapse">
                                <div class="panel-body">
                                    <span class="loader" style="margin-left: 35%;">
                                        <?= Html::img('@web/img/modal_loading.gif', ['alt' => Yii::t('app', 'Loading...')]) ?>
                                    </span>
                                    <div id="ospf_diff"></div>
                                </div>
                            </div>
                        </div>
                    </div>
